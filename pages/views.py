...
def edit(request, id):
    obj = QueryInfo.objects.get(id=id)
    context = {
        'name' : obj.Name, # = models.CharField(max_lengt, #h=50)
        'email' : obj.Email, # = models.CharField(max_lengt, #h=100)
        'phone'  : obj.Phone, # = models.CharField(max_lengt, #h=50)
...
def update(request, id):
    ...
    sts = True if sts.strip().lower() == 'true' else False
    QueryInfo.objects.filter(id=id).update(Message=msg, Status=sts)
    return redirect('query_table_page')

def delete(request, id):
    queryinfo = QueryInfo.objects.get(id=id)
    queryinfo.delete()
    return redirect('query_table_page')

def rmd(request, id):
    QueryInfo.objects.filter(id=id).update(Date = datetime.datetime.now())
